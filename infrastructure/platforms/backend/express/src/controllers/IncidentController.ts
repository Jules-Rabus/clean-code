...
export class IncidentController {
  ...
  constructor() {
    ...
    this.findAllIncidentsUseCase = new FindAllIncidentUseCase(
      incidentsRepository,
    );
    this.findOneIncidentUseCase = new FindOneIncidentUseCase(
      incidentsRepository,
    );
  ...
  async getIncidentsByBike(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.error(
        "Erreur dans IncidentController.getIncidentsByBike:",
        error,
      );
      res.status(500).json({ error: "Erreur interne" });
    }
