...
export class BikeController {
  ...
  async getBike(req: Request, res: Response): Promise<void> {
    try {
      ...
      const bike = await this.findOneBikeUseCase.execute(
        new VinIdentifier(vin),
      );
      if (!bike) {
  ...
  async updateBike(req: Request, res: Response): Promise<void> {
    try {
      const { vin } = req.params;
      const bikeData = req.body;
      const bike = await this.updateBikeUseCase.execute(
        new VinIdentifier(vin),
        bikeData,
      );
      if (!bike) {
  ...
  async removeBike(req: Request, res: Response): Promise<void> {
    try {
      const { vin } = req.params;
      const success = await this.removeBikeUseCase.execute(
        new VinIdentifier(vin),
      );
      if (!success) {
        res.status(404).json({ error: "Moto non trouvée" });
